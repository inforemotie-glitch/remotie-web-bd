/* =====================================================================
   ...
   06 Pointer effects             12 Meta Pixel + CAPI
                                  13 Boot
   ---------------------------------------------------------------------
...
/* ------------------------------------------------------------------ */
/* 12 · META PIXEL + CONVERSIONS API  (hybrid, deduplicated)           */
/* ------------------------------------------------------------------ */
/* Every event travels on two rails. The Pixel sends it from the
   browser, /api/meta sends it again from the server with the visitor's
   IP and user agent. Both carry the SAME eventID, which is how Meta
   collapses the pair into one event instead of counting two. The id is
   therefore generated exactly once per action and shared, never
   regenerated for the second rail. */

const META_ENDPOINT = "/api/meta";

/** RFC 4122 version 4 UUID. Uses crypto.randomUUID where available. */
function uuidv4() {
...
/** Read one cookie, or "" when it is not set. */
...
/* _fbp is the Pixel's browser id. _fbc is the ad click id and is only
   written once the Pixel has run, so rebuild it from ?fbclid= when the
   cookie is not there yet. Both lift match quality on the server rail. */
function fbcValue() {
...
/**
 * Fire one event on both rails under a single shared eventID.
 * @param  {string} eventName   Meta standard event, e.g. "PageView", "Lead".
 * @param  {object} [customData] value, currency, content_name, ...
 * @param  {object} [userData]   { email, phone }. Sent only to our own
 *         server, which SHA-256 hashes it before Meta ever sees it.
 * @return {string} the eventID that both rails used.
 */
function trackMeta(eventName, customData = {}, userData = {}) {
  ...
  /* rail 2 · server, same eventId → Meta deduplicates the pair */
  try {
    fetch(META_ENDPOINT, {
      method: "POST",
...
/* Exposed so inline handlers and the contact form can reuse it. */
window.trackMeta = trackMeta;

function initMetaTracking() {
  /* PageView. The base code in index.html deliberately does not call
     it, so this is the only PageView on the page. */
  trackMeta("PageView");

  /* Lead. One delegated listener covers every primary CTA on the page,
     header, hero, service cards, contact card and footer. */
  document.addEventListener("click", e => {
    const cta = e.target.closest && e.target.closest("[data-book]");
    if (!cta) return;
    trackMeta("Lead", {
      content_name: (cta.textContent || "Book a meeting").replace(/\s+/g, " ").trim(),
      content_category: "booking_cta"
    });
  }, { passive: true });
}
